        ...
        while (reaped == 0 and wait) {
            ...
            for (kqueue.events[0..kqueue_events]) |event| {
                ...
                const result: Result = blk: {
                    switch (job.type) {
                        .wake => {
                            assert(event.filter == std.c.EVFILT.USER);
                            assert(event.ident == WAKE_IDENT);
                            job_complete = false;
                            break :blk .wake;
                        },
                        .timer => |inner| {
                            assert(event.filter == std.c.EVFILT.TIMER);
                            assert(inner == .none);
                            break :blk .none;
                        },
                        .accept => |*inner| {
                            assert(event.filter == std.c.EVFILT.READ);
                        ...
                        .connect => |inner| {
                            assert(event.filter == std.c.EVFILT.WRITE);
